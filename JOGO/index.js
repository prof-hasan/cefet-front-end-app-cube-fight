...
canvas.width = 1671; //1600
canvas.height = 940; //900
...
class Item {
    constructor({ x, y, width, height, velocidade, arma, orientacao }) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
        ...
        // velocidade de rotação (radians por frame) dependendo do tipo de arma
    ...
    desenhar() {
        if (this.arma == 'lanca' || this.arma == 'luva' || this.rodando == false || this.velocidade.x == 0) {
            if (this.orientacao == 'esquerda') {
                c.save();
                c.scale(-1, 1);
    ...
    update() {
        ...
        if (this.velocidade.x < 1 && this.velocidade.x > -1) this.pegavel = true;
    ...
    deletarArma() {
        setTimeout(() => {
            this.deletar = true;
        }, 15000); //arma dura 15seg antes de sumir
    }
}
...
function tocarSom(som) {
    console.log(som)
    let audio = new Audio('../EfeitosSonoros/' + som + '.wav')

    audio.play()
    // audios.push(new Audio('../EfeitosSonoros/'+ som + '.wav'));

    // console.log()

    // let audioAtual = audios[audios.length-1];

    // audioAtual.play();

    // audioAtual.addEventListener('loadedmetadata', () => {
    //     setTimeout(() => {
    //         audios.splice(audios.indexOf(audioAtual), 1);
    //         console.log("deletou")
    //     }, audioAtual.duration);
    // });

    // audioAtual.addEventListener('loadedmetadata', () => {
    //     let duration = audioAtual.duration; // Duration in seconds
...
for (let j = 0; j < 2; j++) {
    switch (arrayDosPlayers[j].chapeu) {
        case 'rei':
            ...
            break;
        case 'mago':
            jogadores[j].gravidadeBase *= 0.80; //diminui em 25%
            break;
        case 'paleto':
            jogadores[j].ataqueCooldown *= 0.75; //aumenta em 75%
            break;
        case 'cowboy':
    ...
    switch (arrayDosPlayers[j].rosto) {
        ...
        case 'feliz':
            ...
            break;
        case 'neutro':
            jogadores[j].dashCooldown -= 100; //aumenta em 75%
            break;
        case 'surpreso':
    ...
    for (let i = 0; i < 2; i++) { //i assume 0 e 1 (só podem ter 2 jogadores)
        ...
        if (teclas[i].ataque.pressionada && jogadores[i].podeAtacar) { //dispara a animação de ataque sempre que apertar (com cooldown)
            ...
            if (outroJogador.position.y + outroJogador.height >= jogadores[i].position.y - jogadores[i].alcanceVerticalBase && outroJogador.position.y <= jogadores[i].position.y + jogadores[i].height + jogadores[i].alcanceVerticalBase) {//condição de proximidade para o ataque (jogadores alinhados verticalmente)
                ...
                if (ataqueValido) { //efeitos do ataque
                    ...
                    jogadores[i].boostDano += jogadores[i].ataqueBase / 10; //aumenta o knockback a cada hit em 10%
        ...
        if (teclas[i].arremesso.pressionada && jogadores[i].arma != "vazio" && jogadores[i].podeArremessar) {
            ...
            armas.push(new Item({
                ...
                velocidade: {
                    x: jogadores[i].orientacao == 'direita' ? jogadores[i].velocidade.x / 2 + velocidadeDoArremesso : jogadores[i].velocidade.x / 2 - velocidadeDoArremesso,
                    y: -4,
                },
...
window.addEventListener('keydown', ({ code }) => {
    switch (code) {
        ...
        case 'ArrowRight': // apertou DIREITA
            ...
            break;
        case 'Numpad1': // apertou 1 direito
            teclas[1].ataque.pressionada = true;
            break;
        case 'Numpad2': // apertou 2
            teclas[1].dash.pressionada = true;
            break;
        case 'Numpad3': // apertou 3 (arremessar)
...
function definirImagemVencedor(array, vencedor) {
    ...
    // pausa o contador de tempo e registra o valor final
    jogoRodando = false;

    const totalMs = Math.round(tempoDeJogoMs); //tempo total em milissegundos

    const minutos = Math.floor(totalMs / 60000); //minutos
    const segundos = Math.floor((totalMs % 60000) / 1000); //segundos
    const milissegundos = totalMs % 1000; //milissegundos

    //printa o tempo formatado
    console.log( 
    ...
    let outroJogador = jogadores[vencedor == 0 ? 1 : 0]; //pega o outro jogador

    console.log("dano causado: " + outroJogador.danoRecebido) //o dano causado de um é o dano recebido do outro
    console.log("dano recebido: " + jogadores[vencedor].danoRecebido) //dano recebido do vencedor
...
const botaoRejogarEl = document.querySelector("#btn-rejogar");
const botaoVoltarMenuEl = document.querySelector("#btn-voltarMenu");

botaoRejogarEl.addEventListener("click", function () {
